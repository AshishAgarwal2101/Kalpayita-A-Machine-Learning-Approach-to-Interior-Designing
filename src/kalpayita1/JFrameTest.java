...
public class JFrameTest extends javax.swing.JFrame {
    ...
    Node1 root;
    HashMap<String,Node1> nodes;
    HashMap<Integer, ArrayList<Node1>> levelNodes; 
    ...
    String inferObjsBedRoom[]={"bed","nothing"};
    ...
    private void initCoreVariables(){
        ...
        toBeObjects = new String[]{"wall", "notepad", "side-table", "sandwich", "flower-pot", "wall-hanging", "plate", "bed", "nightstand","floor-lamp"};
        toBeObjectsPlural = new String[]{"walls", "notepads", "side-tables", "sandwiches", "flower-pots", "wall-hangings", "plates", "beds", "nightstands"};
    ...
    private void initVariables(){
        ...
        root = null;
        nodes = new HashMap<>();
        levelNodes = new HashMap<Integer, ArrayList<Node1>>();
        
        scene_type = ""; //"bedroom" just for initialisation purpose, changed later
    ...
    /*public void AcceptListener(java.awt.event.ActionEvent evt, String st)
    {
        System.out.println(st);
        jButton1ActionPerformed(evt);
    }*/
    public void jButton1ActionPerformed(java.awt.event.ActionEvent evt, String st){                                         
        // TODO add your handling code here:
        //HelloJME3.processingDialog.setVisible(true);
        Thread stopper = new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    doWork(text);
                    HelloJME3.root = root;
                    HelloJME3.levelInfo = levelNodes;
                    HelloJME3.recreate = false;
                    HelloJME3.updateScene();
        ...
        //HelloJME3.processingDialog.setVisible(false);
        //System.out.println(st);
    ...
    public void initializeScenesAndDescriptions(){
        //System.out.println("Entered scenes and descriptions");
        JSONParser parser;
        ...
        //System.out.println("Bedroom sentences: "+bedroom_sentences);
    }
    public void initRelationsMapping(){
        ...
        try{
            ...
            JSONObject o2 = (JSONObject)o1.get("bedroom"); //since we have given same relations type to all scene types
                ...
                if((containsElement(toBeObjects, lem) || containsElement(toBeObjectsPlural, lem) || pos.equals("NN") || pos.equals("NNS")) && !relationsMapping.containsKey(word)){
                    ...
                    if(!nouns.contains(lem)){
                        ...
                        if(!(containsElement(toBeObjects, lem) || containsElement(toBeObjectsPlural, lem)) && !physicalObjFinding(lem)){
                            //System.out.println("Adding to other nouns: "+lem);
                            otherNouns.add(lem);
    ...
    public void initializeSceneType(String text){
        //System.out.println("In scene type;;; living_room_sentences: "+living_room_sentences);
        String st="";
        String scene_type1="";
        ...
        //scene type inference from probability formula
         if(!(scene_type1.equals("bedroom")|| scene_type1.equals("office")|| scene_type1.equals("living-room")))//infer scene type
        {
            //System.out.println("scene_type further calculation");
         ...
         scene_type= scene_type1;
         //System.out.println("Calculated Scene Type = "+scene_type);
    }
    public List<CoreMap> sceneParsingStanfordPipelineInitialisation(String text)
    {
    ...
    public void sceneParsing(String text){
        ...
        try{
            ...
            List<CoreMap> sentences = sceneParsingStanfordPipelineInitialisation(text);
            initRelationsMapping();
    ...
    {
        //System.out.println("In scene inference: bedroom sentences: "+bedroom_sentences);
        if(scene_type.equals("living_room"))
        {
        ...
        {
           /* if((objects1.contains(s)||objects2.contains(s)) &&
                    (relationsList.contains("supported_by")||relationsList.contains("supports"))){
    ...
    public void createSceneGraph(){
        for(String obj:objAttrWithType.keySet()){
            String attr = objAttrWithType.get(obj)[0];
            String type = objAttrWithType.get(obj)[1];
            Node1 node = new Node1(obj, attr, type);
            nodes.put(obj, node);
            if(obj.equals("room")){
                root = node;
            }
        ...
        for(String rel:relations.keySet()){
            ...
            for(String[] objects:relObjects){
                String obj1 = objects[0];
                String obj2 = objects[1];
                Node1 node1 = nodes.get(obj1);
                Node1 node2 = nodes.get(obj2);
                if(!rel.equals("supports") && !rel.equals("supported_by") && !node1.next.contains(node2)){
                    node1.next.add(node2);
                    node1.connectedAs.add(rel);
                }
    ...
    public void giveLevelsToNodes(Node1 root, int currLevel){
        root.level = currLevel;
        ArrayList<Node1> nodesOnThisLevel;
        ...
        for(Node1 node:root.next){
            ...
            if(connectedAs.equals("supports")){
                if(!levelNodes.containsKey(currLevel + 1)){
                    ...
                    ArrayList<Node1> nodesOnNextLevel = levelNodes.get(currLevel+1);
    ...
    public void displaySceneGraph(){
        ...
        for(int level:levelNodes.keySet()){
            ArrayList<Node1> nodes = levelNodes.get(level);
            System.out.print("Level "+level);
            for(Node1 node:nodes){
                if(node.attributeType == null){
                    System.out.print("\t|\t"+node.name+"\t|\t"+node.attributeType+"\t\t|\t");
                }
                else{
                    System.out.print("\t|\t"+node.name+"\t|\t"+node.attributeType+"("+node.attribute+")\t|\t");
                }
                ArrayList<Node1> connectedNodes = node.next;
                ArrayList<String> connectedAs = node.connectedAs;
                int ii=0;
                for(Node1 connectedNode:connectedNodes){
                    String connected = connectedAs.get(ii++);
                    System.out.print(node.name+"-"+connected+"-"+connectedNode.name);
                    if(ii != connectedAs.size()){
                        System.out.print(", ");
                    }
                }
                System.out.println();
    ...
    public boolean containsElement(String[] s1, String s){
